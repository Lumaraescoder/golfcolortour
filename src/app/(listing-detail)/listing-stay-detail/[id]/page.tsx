...
const ListingStayDetailPage: FC<ListingStayDetailPageProps> = ({ }) => {
  ...
  // canonical gallery is `dataItem.gallery` (array of {src, alt})
  const gallerySource: any[] = dataItem && Array.isArray(dataItem.gallery) ? dataItem.gallery : [];
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "ArrowRight") {
        setGalleryIndex((i) => Math.min(gallerySource.length - 1, i + 1));
        ...
        <div className="relative grid grid-cols-3 sm:grid-cols-4 gap-1 sm:gap-2">
          ...
          >
            <Image
              ...
              src={gallerySource[0]?.src || ""}
              alt={gallerySource[0]?.alt || ""}
              sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 50vw"
            />
            <div className="absolute inset-0 bg-neutral-900 bg-opacity-20 opacity-0 hover:opacity-100 transition-opacity"></div>
          </div>
          {gallerySource.slice(1, 5).map((item, index) => (
            <div
              key={index}
                ...
                <div className="relative bg-black rounded-md overflow-hidden">
                  ...
                  <div className="flex items-center justify-center h-[70vh]">
                    ...
                    <div className="max-h-full max-w-full flex items-center justify-center">
                      <Image
                        src={gallerySource[galleryIndex]?.src || ""}
                        alt={gallerySource[galleryIndex]?.alt || `photo ${galleryIndex + 1}`}
                        width={1280}
                        height={800}
                    ...
                    <button
                      ...
                      onClick={() => setGalleryIndex((i) => Math.min(gallerySource.length - 1, i + 1))}
                      aria-label="Next"
                    >
                      ›
                    </button>
                  ...
                  <div className="flex gap-2 overflow-x-auto p-3 bg-black/60">
                    {gallerySource.map((u, i) => (
                      ...
                      >
                        <Image src={u?.src || ""} alt={u?.alt || `thumb ${i + 1}`} width={160} height={100} className="h-20 object-cover" />
                      </button>
                    ))}
                  </div>
                </div>
